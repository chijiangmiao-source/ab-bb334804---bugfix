#!/bin/sh
# 一次性验收：单元测试 -> 规范裁决回归 -> 构建检查 -> HTTP 冒烟。
...
echo "== [2/4] 规范裁决回归（前两级同优分段场景） =="
if ! node verify/canonical.mjs; then
  echo "VERIFY FAILED: 规范路径裁决不正确 (exit 1)"
  exit 1
fi

echo "== [3/4] 构建检查（tsc 类型检查 + vite 构建） =="
if ! npm run build; then
  echo "VERIFY FAILED: 构建检查未通过 (exit 20)"
  exit 20
fi

echo "== [4/4] HTTP 冒烟（${BASE_URL:-http://web:80}） =="
